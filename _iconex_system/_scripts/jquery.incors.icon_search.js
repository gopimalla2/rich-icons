/*  Iconexperience Search, version 2.2
 *
 *  (c) 2013 Incors GmbH
 *--------------------------------------------------------------------------*/
...
SEARCH_KEYWORD_PROCESSOR = typeof SEARCH_KEYWORD_PROCESSOR === 'undefined' ? function(x) { return x; } : SEARCH_KEYWORD_PROCESSOR;

SEARCH_REPLACEMENT_MAP = typeof SEARCH_REPLACEMENT_MAP === 'undefined' ?  {} : SEARCH_REPLACEMENT_MAP;

HIDE_PLAIN_AND_SHADOW_IN_PATH = typeof HIDE_PLAIN_AND_SHADOW_IN_PATH === 'undefined' ? false : HIDE_PLAIN_AND_SHADOW_IN_PATH;
  ...
  */
  resultIconsDelayed: function(value, callBack, showAllResults) {
    ...
    if ($('#search_type_icon_names:checked').val()) {
      ...
      value = SEARCH_KEYWORD_PROCESSOR(value);
...
$.Incors.Image = {
  imgPlainPath: function(name, size, colorStyle) {
    ...
    var plain = HIDE_PLAIN_AND_SHADOW_IN_PATH ? '/' : '/plain/';
  ...
  imgShadowPath: function(name, size) {
    var colorStyle = colorStyle ? colorStyle : (typeof(DEFAULT_COLOR_STYLE) == 'undefined') ? null: DEFAULT_COLOR_STYLE;
    var imgPath = colorStyle ? IMG_PATH + '/' + colorStyle : IMG_PATH;
    var shadow = HIDE_PLAIN_AND_SHADOW_IN_PATH ? '/' : '/shadow/';
...
$.Incors.Template.resultTmpl = function(vars) {
  return ['<table id="results_table" cellspacing="0" cellpadding="0"><colgroup><col width="150px" /><col width="100%" /></colgroup>' , vars.resultRows , '</table>'].join('');
};

$.Incors.Template.resultRowTmpl = function(vars) {
  return ['<tr class="results_row_' , vars.oddOrEven , '"><td class="results_col_left"><table cellspacing="0" cellpadding="0" class="results_col_left_text" style="background: ', vars.backgroundColor , ';"><tr><td>' , vars.keyword , '</td></tr></table></td><td class="results_col_right"><ul class="icon_list_1">' , vars.resultListImgs , '</ul></td></tr>'].join('');
};
...
$.Incors.Template.noKeywordResultTmpl = function(vars) {
  return ['<table cellspacing="0" cellpadding="0" id="no_results"><tr><td><div id="no_result_text">No keywords found matching: "', vars.searchWord, '"</div></td></tr></table>'].join('');
};

$.Incors.Template.noNameResultTmpl = function(vars) {
  return ['<table cellspacing="0" cellpadding="0" id="no_results"><tr><td><div id="no_result_text">No icon names found matching: "', vars.name, '"</div></td></tr></table>'].join('');
};
...
$.Incors.Template.detailsTmpl = function(vars) {
  return [
    ...
    '<table id="details_table" cellspacing="0" cellpadding="0"><tr>' ,
    '<td style="width: 50%;">' ,
